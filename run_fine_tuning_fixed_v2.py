"""
This script fine-tunes a GPT-2 Medium model on TextWorld data with memory optimizations.
...
sft_config.batch_size = 2  # Further reduced batch size
sft_config.learning_rate = 2e-5
sft_config.mixed_precision = True
sft_config.gradient_accumulation_steps = 16  # Increased gradient accumulation
...
if torch.cuda.is_available():
    ...
    if gpu_mem > 40:  # For A100 or similar high-memory GPUs
        ...
        sft_config.batch_size = 2
        sft_config.gradient_accumulation_steps = 16
    else:  # For smaller GPUs
        sft_config.batch_size = 1
        sft_config.gradient_accumulation_steps = 32
